...
身份发现，不接收签名或公证密钥，并跳过 macOS 装订和签名验证。未签名打包仍会对外层应用做 adhoc 签名，使 codesign Identifier 等于 `net.aiuo.pi-desktop`（issue #524）。嵌套 helper 与 framework 完全没有签名时（Intel 通道的 Electron bundle），先按由深到浅的顺序对这段嵌套代码做 adhoc 签名，且不使用 `--deep`（ADR 0278）。如需明确签名，
