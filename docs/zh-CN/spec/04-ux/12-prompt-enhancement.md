...
改写使用哪个模型、用多大思考强度，位于设置 → AI 中一张独立卡片「增强提示词」内，
紧挨「提示词增强」卡片之后。该卡之所以自带标题，是因为它包含一个模型选择器——与默认
模型行一样，需要标题、当前值与控件，无法套用提示词卡那种开关加图标按钮的行形态。
...
该卡片的模型行标题为「默认模型」，使用与设置 → 模型「默认项」卡片相同行的锚定可搜索菜单，因此两处只呈现一种模型选择交互。
