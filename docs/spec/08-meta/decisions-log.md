...
- Decision D364 amends D120 / D126 / ADR 0022: the Windows x64 lane publishes
  ...
  data directory. Portable requests user execution level. The target sets
  `unpackDirName` to `PI-Desktop-Portable`, giving each user a stable extraction
  path for taskbar grouping and pinning; concurrent launches of that artifact
  under one user are unsupported.
